 ...
 */
export default function applyCoupon (packageItem, coupons) {
  if (isValidCoupon(packageItem, coupons) && packageItem.totalCost > 0) {
    const coupon = findCoupon(packageItem.offerCode, coupons)
    return _applyOfferToPackage(packageItem, coupon)
  }
  return packageItem
}
