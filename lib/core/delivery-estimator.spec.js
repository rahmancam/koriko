...
describe('Delivery Estimator', () => {
  ...
  test('Should deduct Tax', () => {
    ...
    function estimatorWithTaxDeduction (options) {
      ...
      return {
        ...packageItem,
        totalCost: packageItem.totalCost - packageItem.totalCost * 0.18
      }
    }

    const packagesWithTax = estimator.estimateCost([makePackage(newPackage)], estimatorWithTaxDeduction)
    expect(packagesWithTax[0].totalCost).toBe(545.3)
  })
  ...
  test('Should estimate time correctly', () => {
    ...
    const shipments = estimator.estimateTime([makePackage(newPackage)], fleetDetail)
    expect(shipments[0].getWeightkg()).toBe(10)
  })
})
